...
private enum WidgetType {
    static let circularScore: Font = .system(size: 18, weight: .bold).monospacedDigit()
    static let rectangularScore: Font = .system(size: 22, weight: .bold).monospacedDigit()
    static let cornerScore: Font = .system(size: 20, weight: .bold).monospacedDigit()
    static let inline: Font = .system(size: 14, weight: .semibold).monospacedDigit()
        ...
        HStack(spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text(entry.displayScore)
                    ...
                    .foregroundStyle(entry.score == nil ? WidgetPalette.fgMuted : WidgetPalette.fg)
                if let stateLabel = entry.stateLabel {
                    Text(stateLabel)
                        .font(WidgetType.monoTag)
                        .tracking(WidgetType.monoTagTracking)
                        .foregroundStyle(entry.scoreColor)
