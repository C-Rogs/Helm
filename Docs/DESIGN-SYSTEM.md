...
- **AdjustmentBanner (provenance)**: the single treatment for any plan change (readiness gating, flagged context, in-session swap). Accent-tinted surface, a change glyph, `from -> to` in `fg`, a reason in `body`, and an `UNDO` affordance. Always logged to `coach_recommendation`; always fires the coach-adjust haptic.
- **AskCoachBar**: bottom-anchored, thumb-reach, a pulsing accent dot plus prompt text. Present on Dashboard and in-session.
